...
export default function FilterBar({
  ...
  const toggleYear = (value: string) => {
    if (yearsFilter.includes(value)) {
      setYearsFilter(yearsFilter.filter((y) => y !== value));
    } else {
      setYearsFilter([...yearsFilter, value]);
    }
  };
    ...
    <Box>
      ...
      >
        {YEAR_OPTIONS.map((option) => {
          const isSelected = yearsFilter.includes(option.value);
          return (
            <Button
              key={option.value}
